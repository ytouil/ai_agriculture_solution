...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    foreach ($lines as $line) {
        ...
        if (json_last_error() === JSON_ERROR_NONE) {
            if (isset($data['error'])) {
                $errors[] = $data['error'];
                if (isset($data['traceback'])) {
                    $errors[] = "Traceback: " . $data['traceback'];
                }
    ...
    $template_path = '/opt/lampp/htdocs/websiteNasa/resultNASA_template.html';
    $html_content = file_get_contents($template_path);
    
    if ($html_content === false) {
        echo "Error: Could not load the resultNASA_template.html file from path: $template_path";
        exit();
    }
    ...
    if (file_put_contents($result_file_path, $html_content) === false) {
        echo "Error: Failed to write resultNASA.html file. Check permissions for path: $result_file_path";
        exit();
    }
